...
import { regions, getSentimentColor, getSentimentLabel } from "@/lib/dashboardData";
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Real-time civic mood by region</p>
        </div>
        <SentimentLegend />
      </div>
      ...
      <div className="relative aspect-[4/3] bg-muted/30 rounded-xl overflow-hidden border border-border/50">
        ...
        {regions.map((region, index) => (
          ...
          >
            ...
            {/* Main dot */}
            <div
              className="relative w-5 h-5 rounded-full border-2 border-background shadow-lg"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">
        {regions.slice(0, 4).map((region) => (
          <div key={region.id} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30">
            <div
